...
class ChromaVectorStorageConnection(VectorStorageConnection):
    """ChromaDB 存储连接实现"""
    ...
    def connect(self) -> bool:
        ...
        logger.info("ChromaVectorStorageConnection connected")
        return True
    
    def disconnect(self) -> None:
        self._connected = False
        logger.info("ChromaVectorStorageConnection disconnected")
    ...
    def delete_vectors(self, ids: list[str]) -> bool:
        try:
            self._storage.delete_vectors(ids)
            return True
        except Exception as e:
    ...
    def update_vectors(self, items: list[EngineVectorItem]) -> bool:
        try:
            self._storage.update_vectors(items)
            return True
        except Exception as e:
    ...
    def search(
        ...
        raise NotImplementedError("Search should be done through QueryEngine, not storage connection")


class ChromaVectorTransaction(BaseVectorTransaction):
    """ChromaDB 向量事务实现（应用层 UnitOfWork）"""
    ...
    def __init__(
        ...
        logger.info(f"ChromaVectorTransaction created: {self._transaction_id}")
    ...
    def begin(self, options: Optional[TransactionOptions] = None) -> None:
        ...
        self._deleted_ids = []
        self._savepoints = {}
        logger.info(f"ChromaVectorTransaction {self._transaction_id} started")
    ...
    def commit(self) -> TransactionResult:
        ...
        try:
            ...
            self._clear_pending()
            logger.info(f"ChromaVectorTransaction {self._transaction_id} committed")
            ...
            self._clear_pending()
            logger.error(f"ChromaVectorTransaction {self._transaction_id} commit failed: {e}")
    ...
    def rollback(self, reason: Optional[str] = None) -> TransactionResult:
        ...
        logger.info(f"ChromaVectorTransaction {self._transaction_id} rolled back: {reason}")
...
class ChromaVectorTransactionManager(BaseVectorTransactionManager):
    """ChromaDB 向量数据库事务管理器实现"""
    
    def __init__(self, storage_connection: VectorStorageConnection):
        self._storage_connection = storage_connection
        self._current_transaction: Optional[ChromaVectorTransaction] = None
    ...
    def create_transaction(
        ...
        return ChromaVectorTransaction(transaction_id, self._storage_connection, options)
