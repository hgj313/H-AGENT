...
class MetadataExtractorNode:
    ...
    def _extract_policy_numbers(text: str, file_name: str = "") -> tuple[str, str, str]:
        ...
        patterns = [
            ...
            r"保险单号码?[：:\s]*([A-Z0-9]{16,30})",              # 3. 利宝"保险单号"/北部湾"保险单号码"（2026-09-22 增"码"兼容）
    ...
    def _is_real_list_page(text: str) -> bool:
        """判断是否真的"人员清单"页（含人员表头词 + 证件号模式）

        2026-09-22 修正：北部湾"雇主责任保险电子保单"第1页是保险单主页，
        含"证件号码：91500115MA5U6FB90C"（被保险人的**统一社会信用代码**，非人员身份证），
        旧逻辑仅凭"证件号码"就把主页误判为清单页。
        修正："姓名/雇员姓名"才是人员清单的通用表头；若无姓名但有"证件号"，
        仅当页面**不含"统一社会信用代码"**时才视为清单页（兼容旧格式兜底）。
        """
        has_header = any(kw in text for kw in ["姓名", "雇员姓名"])
        if not has_header:
            # 兜底：无"姓名"表头，但有"证件号/证件号码"标签
            # 排除"统一社会信用代码"（那是被保险人公司证件，不是人员身份证）
            has_header = (
                any(kw in text for kw in ["证件号", "证件号码"])
                and "统一社会信用代码" not in text
            )
