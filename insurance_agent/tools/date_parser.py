...
_OVERALL_PATTERNS = [
    re.compile(
        r"自(\d{4})年(\d{2})月(\d{2})日[0-9时分秒\s]*起[,，]?\s*至(\d{4})年(\d{2})月(\d{2})日[\d时分秒\s]*止"
    ),
    re.compile(
        r"自(\d{4})-(\d{2})-(\d{2})[\s\d:]*起至(\d{4})-(\d{2})-(\d{2})[\s\d:]*止"
    ),
    re.compile(
        r"保险期间[：:\s]*自(\d{4})[年\-](\d{2})[月\-](\d{2})[日\s\d:]*至(\d{4})[年\-](\d{2})[月\-](\d{2})"
    ),
]

# 单点日期：2026-06-24 00:00:00  / 2026年06月24日
_DATE_PATTERN = re.compile(
    r"(\d{4})[-年](\d{2})[-月](\d{2})[日\s]*(?:\d{1,2}[时:]\d{1,2}(?:分:?\d{1,2}秒?)?)?"
)
