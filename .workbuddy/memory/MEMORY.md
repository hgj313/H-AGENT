...
| **黄河财险-扫描件雇主责任险(主保单 + 批单)** | table(序号/姓名/身份证号/职务/工种类型/**共同被保险人**/风险等级) | ❌整体期限 | ✅"共同被保险人"列=用工单位 | 纯增保(主) / 增保(批) | **2026-09-20 新发现格式**：保单号 `61301041219202600000000162`（黄河财产保险2026版A款雇主责任险）。**全扫描件**(无文字层)→ 走 OCR（`format_hint="ocr"`）。特点：① 单张保单容纳多家被派遣单位(同一保单下"共同被保险人"列多达16家公司，如山东诚晟陶瓷/淄博集成建筑陶瓷/重庆筑起/龙岩禾洛人力/威海茂成达/荣成市鑫军船舶等)；② 风险等级1-5；③ 多达560人、保单33页以上；④ "职务/工种类型"列易被 OCR 误填为 company(如"船体制造工"非公司名) → 必须在 prompt 强调"共同被保险人=company"且用 `_looks_like_company_name` 校验。**OCR prompt 已扩展**（2026-09-20）：新增 `job_title`(职务/工种类型) + `occupation_class`(风险等级) 可选字段；映射到 InsuredPerson 同名 dataclass 字段 |
| **黄河财险-扫描件雇主责任险(批单)** | 同上(table 雇员清单,仅列变动人员) | ❌批改生效日 | ✅ | ✅同上 | **2026-09-20 同上**:批单格式与主保单一致(`筑起·李光亮.pdf`是示例,序号8表示第8次批改,新增1人 李光亮)。批改起止 `2026-09-19 ~ 2026-09-30`,批单号 `61301041219202600000000162-019`。OCR 提取后用主保单 start/end_date 兜底 + endorsement_effective_date 标记 |
...
- **黄河财险扫描件（2026-09-20 新增）OCR 提取要点**：
  1. **场景判定**：PDF 文字层全空 → `is_scanned=True` → metadata 决策 `format_hint="ocr"`，无需新增格式识别分支。
  2. **公司名识别**：表中有"共同被保险人"列才是 company，"职务/工种类型"列(如 船体制造工/砌筑工/陶瓷原料准备工) 是 `job_title` 不是 company。OCR prompt 必须明确提示。
  3. **多公司跨页**：保单可容纳多家被派遣公司，跨页时 company 不同。OCR 现有跨页"detected_company"回填逻辑可能误覆盖为首页公司名 → `_looks_like_company_name` 校验是兜底（必须含"公司/有限/集团/合作社/事务所"任一关键词 + 不含"人员/工种/职业/工人/操作/安装/施工/维修"）。
  4. **OCR prompt 扩展**：增加 `job_title`(职务/工种类型) 和 `occupation_class`(风险等级) 两个可选字段 → 映射到 InsuredPerson 同名字段。
...
## 关键 Bug 修复要点（按时间倒序）
- **万年县盛美主保单上传暴露 4 条历史孤儿批单数据 + 2 层代码 Bug（2026-09-18, commit b61d325）**：用户上传 `万年县盛美建筑工程有限公司_8116013100260112989000_BD(1).pdf` 后，前端"日期异常"检测 (`checkDateAnomaly: !sd || !ed`) 暴露 4 条历史孤儿批单的 NULL 日期：
  - 5957 王瑞江 (9004)、5955 张新镇 / 5956 张新罗 (9005)、5940 陈得军 (重庆耀立 9001)
  - **为什么"之前没暴露、一上传就暴露"**：这 4 条是历史上传的批单记录，**上传时对应主保单尚未入库** → validator 找不到主保单 → 跳过日期补全 → NULL 日期写入数据库；前端"日期异常"检测对孤儿批单不显著，**当用户上传主保单、系统能交叉引用时才突然暴露**。
  - **Bug 1（policy_library）**：`find_main_policy_by_company` 按列表顺序返回**第一个匹配**，万年县盛美有旧主保单 `8116013100260107550000` + 新主保单 `8116013100260112989000`，错返回旧保单 → 兼容性校验拒绝 → 仍找不到主保单。修复：新增 `find_main_policy_by_company_compatible(company, batch_policy_number)` 按保单号前缀兼容性**迭代过滤**后返回；`find_main_policy` 在传入 policy_number 时改用新方法。
  - **Bug 2（validator_node）**：`_link_to_main_policy` 用**严格相等** `main_policy.policy_number != policy_number` 校验批单号 vs 主保单号，但利宝批单号 `71...` 与主保单号 `81...` 本应不同 → 严格相等校验**永远误判不一致** → 拒绝日期补全。修复：严格不等时再用 `_policy_numbers_compatible` 二次校验，兼容则放行。
  - **L3 reload 链**：`web_app/server.py _GRAPH_CODE_FILES` 加入 `policy_library.py` 让 reload 触发重建（但 server.py 自身改动仍需重启服务）。
  - **数据修复**：SQL 补全 4 条（备份 `data/app.db.bak.20260918_before_fix_wannian_main_policy_lookup`）。端到端验证：9004/9005 重传后日期正确补全；`GET /api/insurance/inconsistencies` 返回 `count: 0`；全库 0 条 NULL/空日期记录。
  - **服务重启**：旧 PID 29620 reload 后 `signature changed: False`（缓存旧模块）→ `taskkill /PID 29620 /F` → watchdog 自动重启新 PID 45792 → 修复生效。
  - **通用教训**：① 同公司多份主保单必须按**保单号兼容性**迭代过滤，不能"第一个匹配"；② 批单号 vs 主保单号校验用**前缀兼容**而非严格相等（利宝 71/81 首字符差异是设计而非错误）；③ 孤儿数据暴露模式 = 主保单后入库 → validator 现在能关联 → 前端日期异常检测突然显示一批孤儿批单；④ 热重载陷阱：server.py 自身的 `_GRAPH_CODE_FILES` 列表改动也必须**重启服务**才能让 reload 列表更新。
